...
class JvmBuilderProcessor : KotlinAbstractProcessor(), KotlinMetadataUtils {
  ...
  private lateinit var classProto: ProtoBuf.Class
  private lateinit var appliedType: AppliedType

  override fun getSupportedAnnotationTypes() = setOf(annotationName)

  override fun getSupportedSourceVersion(): SourceVersion = SourceVersion.latest()

  private fun ProtoBuf.ValueParameterOrBuilder.getNameUsingNameResolver() = nameResolver.getString(this.name)
  private fun ProtoBuf.TypeParameterOrBuilder.getNameUsingNameResolver() = nameResolver.getString(this.name)
  private fun ProtoBuf.ClassOrBuilder.getNameUsingNameResolver() = nameResolver.getString(this.fqName)
  private fun ProtoBuf.ValueParameterOrBuilder.resolveType() = type.asTypeName(nameResolver, classProto::getTypeParameter, false)
  private fun ProtoBuf.Type.resolveType() = asTypeName(nameResolver, classProto::getTypeParameter, false)
  ...
  private fun generateBuilder(element: Element) {
    ...
    classProto = classData.classProto
    nameResolver = classData.nameResolver
    appliedType = AppliedType.get(element as TypeElement)

    val fqClassName = classProto.getNameUsingNameResolver().replace('/', '.')
    val `package` = classProto.getNameUsingNameResolver().substringBeforeLast('/').replace('/', '.')
  ...
  private fun generatePropertyAndBuilderFunPerProperty(prefix: String, typeName: TypeName) = classProto.constructorList
      .single { it.isPrimary }
      .valueParameterList
      .map { valueParameter ->
        ...
        Pair(PropertySpec.varBuilder(name,
            appliedType.resolver.resolve(type.asNullable()), KModifier.PRIVATE)
  ...
  private fun generateParameterizedTypes() = classProto.typeParameterList
      .map { typeArgument ->
